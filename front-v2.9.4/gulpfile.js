...
function scss1() {
  return gulp
    ...
    .pipe(autoprefixer([
        ...
        "Android >= 4.4",
        "Opera >= 30"], { cascade: true }))
        .pipe(gulp.dest('/Work/Azmanova31/Azmanova.WEB/wwwroot/css/'))
    //    .pipe(gulp.dest('./assets/css/'))
        .pipe(browsersync.stream());
}
...
function watch() {
  ...
  gulp.watch('./assets/scss/**/*.scss', scss1);
  gulp.watch(
    [
      './html/**/*.html',
      './starter/**/*.html',
...
// Gulp Tasks
gulp.task('default', gulp.parallel(watch, scss, scss1, minCSS, browserSync));
...
function minCSS() {
  return gulp
    .src([
      ...
      '/Work/Azmanova31/Azmanova.WEB/wwwroot/css/theme.css'
    ])
    .pipe(cssnano())
